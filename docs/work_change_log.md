...
## [2026-03-04] [cl] OHM 폴리곤 대확장 2차 — 한반도·중국·인도·동남아·페르시아

### 배경
- 1차 보강(아래 항목) 이후 고대 한반도·중국 수동 폴리곤(9000xxx) 품질 문제 지적
- 기원전~5세기 시기 데이터 공백 심각 (특히 고구려·왜국·삼국시대 초기)
- 인도·동남아·중앙아시아·페르시아 등 비동아시아 지역도 폴리곤 부재

### 작업 내용

#### 1. 한반도 폴리곤 교체/추가 (세션 전반)
| 엔티티 | 변경 내용 | rid |
|--------|-----------|-----|
| 고구려 | 21pts→4스냅샷(191/116/100/72pts) | 9100201~204 |
| 왜국 | 30pts→2스냅샷(146/206pts) | 9100211~212 |
| 통일신라 | 22pts→2스냅샷(219/1094pts) | 9100221~222 |
| 발해 | 22pts→75pts | 9100231 |
| 백제 | start_year 371→-18, 2스냅샷 추가 | 9100271~272 |
| 신라 | start_year 400→-57, 2스냅샷 추가 | 9100281~282 |

#### 2. 중국 폴리곤 교체/추가
| 엔티티 | 변경 내용 | rid |
|--------|-----------|-----|
| 동진 | 25pts→218pts | 9100241 |
| 진(秦) 제후국 | 14pts→33pts | 9100251 |
| 흉노 | 26pts→40pts | 9100261 |
| 동주 | 8pts→175pts (Zhow states) | 9100291 |
| 남월국 | 22pts→98pts | 9100292 |
| 한나라 | 168pts→356pts (업그레이드) | 9100101 교체 |

#### 3. 유목민족·인도 폴리곤 추가 (이전 세션)
| 엔티티 | 스냅샷 | rid |
|--------|--------|-----|
| 킵차크 칸국 | 2 (398+106pts) | 9100301~302 |
| 무굴 제국 | 3 (46/266/277pts) | 9100311~313 |
| 위구르 카간국 | 1 (43pts) | 9100321 |

#### 4. 16개 신규 엔티티 대량 추가 (이번 세션 핵심)
| 지역 | 엔티티 | 스냅샷 | 최대 pts |
|------|--------|--------|----------|
| 인도 | 마우리아(Q80026), 굽타(Q83462), 쿠샨(Q164992), 사타바하나(Q372866), 사카(Q1149316), 델리 술탄국(Q160419) | 12 | 196pts |
| 동남아 | 크메르(Q162024), 참파(Q193287), 부남(Q193292), 스리비자야(Q155863) | 12 | 823pts |
| 중앙아시아 | 차가타이 칸국(Q487598), 토번(Q215530), 박트리아(Q131476) | 3 | 243pts |
| 페르시아 | 사산 왕조(Q80702) | 2 | 192pts |
| 중국 | 북위(Q186419), 서하(Q205935) | 4 | 103pts |

#### 5. 렌더링 버그 수정
- **문제**: 새 스냅샷에 `start`/`end`(숫자) 필드 누락 → 모든 연도에서 렌더링됨
- **원인**: TypeScript가 `snap.start ?? -10000` 사용, undefined면 기본값 적용
- **수정**: 모든 스냅샷에 `start`/`end` 정수 필드 + `start_date`/`end_date` 문자열 필드 둘 다 필수

#### 6. CHGIS 조사 (결론: 부적합)
- Harvard CHGIS: 등록 필요, 접근 제한
- 복단대 CHGIS: 실제 다운로드 → MapInfo TAB → GeoJSON 변환 성공
  - **Regime 경계**: 11피처, 푸젠 소국가만 (부적합)
  - **성급 경계**: 489피처, 중국 남동부만, 188 AD~ (부적합)
- `idris-maps/world-historical-gis-data`: basemaps보다 품질 낮음 (부적합)
- **결론**: basemaps가 폴리곤 소스로 여전히 최선

#### 7. Pleiades 고대 도시 데이터 확보
- 42,015개 고대 장소 CSV 다운로드 완료 (34,454개 좌표 보유)
- 12,900개 도시(settlement), 그리스·로마 중심이나 중앙아시아·페르시아도 일부
- 활용 계획: 고대 T2 좌표 보정, unknown region 채우기, 미래 도시 레이어
- 저장: `/tmp/pleiades-places.csv` (25MB)

### 수치
- ohm_index: 572 → **588 엔티티**, **1,912 스냅샷**
- 새 GeoJSON 파일: 33개 (rid 9100331~9100481)
- 이전 세션 포함: 총 약 55개 파일 추가/교체
- rid 범위: 9100201~9100481

### 커밋
- `e4fc811` 한반도+중국 OHM 폴리곤 basemaps 교체
- `639cb51` Fix start/end 숫자 필드 누락
- `076dbb0` 백제/신라 삼국시대 초기 국경선 추가
- `2ecf889` 동주/남월국 교체 + 한나라 업그레이드
- `e4f9336` 유목민족+인도 OHM 폴리곤 basemaps 교체
- `db37f73` 16개 신규 OHM 엔티티 추가 — 인도·중앙아시아·동남아·페르시아

### 남은 9000xxx 수동 폴리곤 (29개)
- 저품질(8-19pts): 피렌체, 올멕, 마케도니아, 부여, 동녕국, 위만조선, 웨식스, 고촉, 제, 연, 하왕조, 대리국, 동프랑크, 아시리아, 초, 고조선, 고대 이집트, 유연
- 중품질(21-40pts): 프랑스령 인도차이나, 선비, 신나라, 돌궐, 진나라, 금나라, 아케메네스
- 고품질(306+): 유고, 바이마르, 일본, 나치독일 (교체 불필요)
...
### 배경
- OHM 데이터만으로는 중세~근세 국경선에 수백 년 단위 공백 존재
- 진형 아이디어: 프랑스 등 해당국 역사지도 데이터를 웹에서 찾아 보충
- [aourednik/historical-basemaps](https://github.com/aourednik/historical-basemaps) (GPL-3.0) 발견
  - GeoJSON, BC~2010, 100년 단위 세계 국경 폴리곤 제공
...
| 엔티티 | 갭 | 추가 스냅샷 | rid 범위 |
|--------|-----|-----------|---------|
| 프랑스 (Q142) | 1050~1766 (716년) | 8 | 9100001~08 |
| 나폴리 왕국 (Q173065) | 1442~1458 + 1504~1714 | 1 + 확장 | 9100011 |
| 스웨덴 (Q34) | 1323~1721 (398년) | 5 | 9100021~25 |
| 신성 로마 제국 (Q12548) | 1201~1678 (447년) | 6 | 9100031~36 |
| 크림 한국 (Q160440) | 1475~1774 (299년) | 2 | 9100041~42 |
| 포르투갈 (Q45) | 1199~1297 (98년) | 1 | 9100051 |
| 동로마 제국 (Q12544) | 1204~1261 (57년) | 1 | 9100061 |
| 카스티야 (Q179293) | 1074~1157 (83년) | 1 | 9100071 |

#### 2. 중국 왕조 수동 폴리곤 교체 (7개)
| 왕조 | 기존 pts | basemaps pts | 개선 |
|------|---------|-------------|------|
| 한나라 (Q7209) | 56 | 168 | 3x |
| 수나라 (Q7405) | 29 | 305 | 10x |
| 당나라 (Q9683) | 51 | 279 | 5x |
| 요나라 (Q42891) | 27 | 67 | 2.5x |
| 송나라 (Q7462) | 33 | 213+256 (북송/남송 분할) | 14x |
| 원나라 (Q7313) | 48 | 1,564 | 32x |
| 명나라 (Q9903) | 59 | 323+453 | 13x |
- rid 범위: 9100101~109, 기존 9000xxx는 인덱스에서 제거

#### 3. 아시아 T1 신규 추가 (5개, OHM 없던 엔티티)
| 엔티티 | 스냅샷 | 시기 | rid 범위 |
|--------|--------|------|---------|
| 델리 술탄 왕조 (Q229411) | 4 | 1206~1526 | 9100111~14 |
| 크메르 제국 (Q201705) | 4 | 802~1431 | 9100121~24 |
| 시암 (Q1081620) | 5 | 1351~1932 | 9100131~35 |
| 티무르 왕조 (Q484195) | 2 | 1370~1507 | 9100141~42 |
| 촐라 제국 (Q151148) | 2 | 848~1279 | 9100151~52 |

### 수치
- 총 43개 GeoJSON 파일 생성 (rid 9100001~9100152)
- ohm_index: 567 → 572 엔티티
- 약 2,500년 분량의 국경선 공백 해소
- 소스: historical-basemaps, basemap 연도별 폴리곤 추출 후 OHM 포맷 변환

### 커밋
- `50812b2` 프랑스 987~1050 OHM 스냅샷 추가
- `f978877` 프랑스 1050~1766 국경선 8개 스냅샷 추가
- `1cb7caf` 나폴리 왕국 1302~1816 국경선 완전 연결
- `ba179e3` T1 엔티티 6개 OHM 갭 일괄 메움
- `d2146fd` 중국 7개 왕조 폴리곤 교체 + 아시아 T1 5개 신규 추가

### rid 네이밍 규칙
- 9000001~9000104: 기존 수동 폴리곤 (OHM 없는 엔티티용)
- **9100001~9100xxx: historical-basemaps 소스** (이번 세션에서 도입)

### 참고
- basemaps 폴리곤은 세계지도 수준 해상도 (100~1500pts)
- OHM 원본(1000~4000pts)보다 단순하지만, 없는 것보다 훨씬 나음
- 청나라/몽골 등 OHM 데이터가 이미 좋은 것은 교체하지 않음
